...
pub fn add(allocator: std.mem.Allocator, args: []const []const u8) !void {
    if (args.len == 0) {
        try eprint(allocator, "Error: No skills specified. Usage: skill add <name> [<name> ...]\n", .{});
        std.process.exit(1);
    }
    ...
    for (args) |skill_name| {
        ...
        if (std.fs.cwd().access(skill_path, .{})) {
            try eprint(allocator, "Error: Skill '{s}' already exists\n", .{skill_name});
            std.process.exit(1);
        } else |err| {
        ...
        if (git.fetchSkill(allocator, cfg, skill_name, skill_path)) {
            try print(allocator, "✓ Added skill '{s}'\n", .{skill_name});
        } else |err| {
...
pub fn remove(allocator: std.mem.Allocator, args: []const []const u8) !void {
    ...
    for (args) |skill_name| {
        ...
        if (std.fs.cwd().access(skill_path, .{})) {} else |err| {
            if (err == error.FileNotFound) {
                try eprint(allocator, "Error: Skill '{s}' not found\n", .{skill_name});
                had_errors = true;
                continue;
            }
            return err;
